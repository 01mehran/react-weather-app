//Icons;
import { IoClose } from "react-icons/io5";
import { FaSearch } from "react-icons/fa";

export const SearchBox = ({
  isSearchBoxOpen,
  toggleSearchBox,
  onHandleSearch,
  location,
  setLocation,
}) => {
  return (
    <div
      className={`absolute top-1.5 left-0 w-full px-4 transition-transform duration-400 ease-in-out z-10 ${
        isSearchBoxOpen ? "translate-y-0" : "-translate-y-[500px]"
      }`}
    >
      <form onSubmit={onHandleSearch} className="relative w-full rounded-xl">
        <button
          type="submit"
          className="text-navy bg-blue/90 absolute top-1/2 left-2 flex -translate-y-1/2 cursor-pointer items-center justify-center rounded-full p-1 text-[16px] shadow-md"
        >
          <FaSearch />
        </button>
        <input
          type="text"
          className="bg-blue/90 text-navy h-12 w-full flex-1 rounded-md border-0 px-10 text-lg font-medium outline-0"
          placeholder="Enter your location..."
          value={location}
          onChange={(e) => setLocation(e.target.value)}
        />
        <button
          className="text-navy bg-blue/90 absolute top-1/2 right-2 -translate-y-1/2 cursor-pointer rounded-full text-2xl shadow-md transition-colors duration-200 hover:text-red-700"
          onClick={toggleSearchBox}
        >
          <IoClose />
        </button>
      </form>
    </div>
  );
};
